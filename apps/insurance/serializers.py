...
class InsurerRegisterSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        user = InsurerUser.objects.create_insurer_user(validated_data['email'], validated_data['password'])
...
class AssessorRegisterSer(serializers.ModelSerializer):
    email = serializers.EmailField(required=True, validators=[UniqueValidator(queryset=User.objects.all())])
    password = serializers.CharField(write_only=True, required=True, validators=[validate_password])
    password2 = serializers.CharField(write_only=True, required=True)
    eval_code = serializers.CharField(write_only=True, required=True)

    class Meta:
        model = AssessorUser
        fields = ('email', 'password', 'password2', 'eval_code')

    def create(self, validated_data):
        user = AssessorUser.objects.create_assessor_user(validated_data['email'], validated_data['password'],
                                                         validated_data['eval_code'])
...
class InsurerSerializer(serializers.ModelSerializer):
    eval_cases = serializers.RelatedField(many=True, read_only=True)
...
class EvaluationCaseSer(serializers.ModelSerializer):
    attachments =serializers.RelatedField(many=True,read_only=True)
